/**
 * Orbit API Proxy — Express server (Railway)
 *
 * Replaces the two PHP CORS relays the original tool used
 * (wxcc_flows_proxy.php and wxcc_functions_proxy.php). Stateless — it
 * doesn't know who the caller is, it just forwards whatever Authorization
 * header/bearer the browser already has to Cisco's APIs and relays the
 * response back with CORS headers added. The actual security boundary is
 * the bearer token itself (validated by Cisco), same as the original.
 *
 * Routes:
 *   GET/POST /flows?url=<encoded Cisco flow-store URL>[&format=json]
 *     Mirrors wxcc_flows_proxy.php — only allows proxying to
 *     https://api.wxcc-us1.cisco.com/. POST defaults to a multipart file
 *     upload (flows:import needs that); pass &format=json for a plain
 *     JSON POST (flows:publish and similar).
 *
 *   POST /functions   body: { action, orgId, bearer, id?, payload? }
 *     Mirrors wxcc_functions_proxy.php — action is one of
 *     "list" | "detail" | "save".
 *
 *   GET /status
 *     Relays the Webex status RSS feed as JSON (replaces
 *     header.php?action=webex-status).
 *
 *   POST /token   body: { code }
 *     Exchanges an OAuth authorization code for an access token. This is
 *     the one route that needs the Client Secret, which is why it has to
 *     run here rather than in the browser — set WEBEX_CLIENT_ID,
 *     WEBEX_CLIENT_SECRET, and WEBEX_REDIRECT_URI as Railway service
 *     variables (never commit them to a file).
 *
 * Deploy with `railway up` from this directory, then set
 * ORBIT_PROXY_BASE in assets/js/orbit-config.js to the resulting
 * Railway public URL.
 */
